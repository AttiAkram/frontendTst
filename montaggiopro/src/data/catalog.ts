...
import { CATEGORY_PHOTOS } from './images'
...
const SELLERS = ['ecommerce', 'ecommerce', 'ecommerce', 'TechStore IT', 'ElettroHub', 'Bottega Digitale']
...
function makeProduct(i: number): Product {
  ...
  return {
    ...
    specs: [
      ...
      ['Codice', `EC-${category.toUpperCase().slice(0, 3)}-${i.toString().padStart(4, '0')}`],
    ],
    price,
    listPrice: fakeList,
    rating,
    ...
    variants,
    images: rotate(CATEGORY_PHOTOS[category], Math.floor(i / CATEGORIES.length)),
    ...
    coupon: r() < 0.25 ? { code: `EC${Math.floor(range(r, 5, 20))}`, off: Math.floor(range(r, 5, 20)) } : null,
