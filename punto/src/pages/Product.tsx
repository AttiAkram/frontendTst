...
const VIEWS = [
  { label: 'Fronte', style: {} },
  { label: '3/4', style: { rotate: -14, scale: 0.95 } },
  { label: 'Dettaglio', style: { scale: 1.6, x: '-10%', y: '8%' } },
  { label: 'Retro', style: { rotateY: 180, scale: 0.92 } },
]
      ...
      <section className="container pdp__top">
        ...
        <div className="gallery">
          <div className="gallery__stage">
            <div className="gallery__grid" aria-hidden />
            <AnimatePresence mode="wait">
              <motion.div
                key={variant.id + view}
                className="gallery__art"
                initial={{ opacity: 0, scale: 0.9, filter: 'blur(8px)' }}
                animate={{ opacity: 1, scale: 1, filter: 'blur(0px)', ...VIEWS[view].style }}
                ...
                dragElastic={0.2}
              >
                <ProductArt category={p.category} color={variant.swatch} />
              </motion.div>
            </AnimatePresence>
            <span className="gallery__meta mono small muted">
              {VIEWS[view].label} · {variant.label} · trascina
            </span>
            <div className="gallery__tags">
              {p.sponsored && <Tag>Sponsorizzato</Tag>}
              {score >= 92 && <Tag tone="deal">Minimo 12 mesi</Tag>}
            </div>
          </div>
          <div className="gallery__thumbs">
            {VIEWS.map((v, i) => (
              <button key={v.label} className={`thumb${view === i ? ' is-active' : ''}`} onClick={() => setView(i)} aria-label={v.label}>
                <motion.span style={{ display: 'block' }} animate={v.style}>
                  <ProductArt category={p.category} color={variant.swatch} />
                </motion.span>
              </button>
            ))}
        ...
        <div className="buybox">
          ...
          {ext.realDiscount && claimed > 0 && (
            <p className={`small ${fake ? 'bad' : 'ok'}`}>
              {fake ? `⚠ Sconto gonfiato: rispetto alla mediana a 90 giorni è ${real > 0 ? `solo −${real}%` : 'un rincaro'}.` : `✓ Sconto reale −${Math.max(real, 0)}% sulla mediana a 90 giorni.`}
            </p>
          )}
          ...
          <div className="buybox__variants">
            ...
            <div className="row-8">
              {p.variants.map((v) => (
                <button key={v.id} className={`swatch swatch--lg${v.id === variant.id ? ' is-active' : ''}`} style={{ background: v.swatch }} onClick={() => setVariant(v)} aria-label={v.label} />
              ))}
            </div>
          </div>
        ...
        {items.map((x, i) => (
          <div key={x.id} className="bundle__item">
            ...
            <Link to={`/p/${x.id}`} className="bundle__art">
              <ProductArt category={x.category} color={x.variants[0].swatch} />
            </Link>
