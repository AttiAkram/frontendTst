...
const TILES: [string, string, string, ReactNode][] = [
  ...
  ['membership', 'PUNTO+', 'Spedizioni gratis e resi estesi', <Sparkles size={20} key="3" />],
...
const ORDERS = [
  { id: 'PT-8KQ2L1', p: PRODUCTS[3], date: Date.now() - 2 * 864e5, status: 'In consegna', step: 3 },
  { id: 'PT-7ZX9A0', p: PRODUCTS[17], date: Date.now() - 12 * 864e5, status: 'Consegnato', step: 4 },
  { id: 'PT-6MB3C7', p: PRODUCTS[42], date: Date.now() - 40 * 864e5, status: 'Reso rimborsato', step: 4 },
]
          ...
          {ORDERS.map((o) => (
            <div key={o.id} className="order">
              <Link to={`/p/${o.p.id}`} className="order__art">
                <ProductArt category={o.p.category} color={o.p.variants[0].swatch} />
              </Link>
      ...
      <section id="indirizzi" className="acc-sec acc-two">
        <div>
          ...
          {[
            ...
            ['Locker', 'PUNTO. Locker — Stazione Centrale', false],
          ].map(([l, a, def]) => (
  ...
  return (
    <section id="membership" className="acc-sec acc-two">
      <div className={`member${member ? ' is-on' : ''}`}>
        <p className="mono up small">PUNTO+</p>
          ...
          {recent.map((id) => {
            ...
            return (
              <Link key={id} to={`/p/${id}`} className="recent__i" title={p.name}>
                <ProductArt category={p.category} color={p.variants[0].swatch} />
              </Link>
            )
          })}
        </div>
